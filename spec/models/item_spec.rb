...
describe Item do
  ...
  describe '#search' do
    it "keyがない場合、全ての商品レコードは表示できる" do
      item = build(:item)
      key = ""
      expect(@search_items.search(:key).length).to eq 1
    end

    it "商品名に一致するレコードを検索できること" do
      item = build(:item, name: "ロボット")
      key = "ロボ"
      expect(@search_items.search(:key).length).to eq 1
    end

    it "商品名に一致しないレコードは検索できないこと" do
      item = build(:item, name: "ロ-ボット")
      key = "robo"
      expect(@search_items.search(:key).length).to eq 0
    end

    it "商品説明に一致するレコードを検索できること" do
      item = build(:item, explanation: "ロボです")
      key = "ロボ"
      expect(@search_items.search(:key).length).to eq 1
    end
